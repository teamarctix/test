...
def stats(status,link,error="No Error"):
    stats = f'<b>├  Status: </b>{status}\n'\
            f'<b>├  Uploaded Videos: </b>{up["Total"]}\n'\
...
def ytdlpp(link):
     time.sleep(2)
     os.system("""yt-dlp --downloader aria2c --match-filter "duration>180" --max-downloads 167 -N 4  --download-archive dled.txt --lazy-playlist  -o '%(title)s.%(ext)s' -f '(mp4)[height=?720]' --write-thumbnail --embed-metadata """ + link)
     print("Download Completed")
     #time.sleep(20)
     up["ytdl"] = True
folder = gof.create_folder(parentFolderId="e470784e-1dad-4712-865a-abf2befcb592",folderName=link.split("/")[-1].capitalize())
...
async def main():
   async with app: 
     ...
     while upl:
            ...
            await app.edit_message_text(-1002034630043,5,text=stats("Active",link,))
                        ...
                        for  filename in files[0]:
                            if filename not in filenames:
                                try:
                                        ...
                                        #await app.send_photo(-1002034630043, photo=filename.replace(".mp4",".png"))
                                        ...
                                        print(e)
                                        await app.edit_message_text(-1002034630043,5,text=stats("Completed",link,e))
                    if up["Total"] > 15:
                            upl = False
            ...
            await app.edit_message_text(-1002034630043,5,text=stats("Offline",link,up["Total"]))
